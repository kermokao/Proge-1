...
class Student:
    ...
    def __init__(self, name, courses: list, grade: float):
        """Student constructor."""
        self.name = name
    ...
    def __repr__(self):
        ...
        return f"Student({self.name}, {self.courses}, {self.grade})"
...
def filter_by_course(student_list: list, course: str) -> list:
    ...
    """
    filtered_list = filter(lambda student: course in student.courses, student_list)
    return [student for student in filtered_list]
...
def sort_by_best_grade(student_list: list) -> list:
    ...
    """
    filtered_average = sorted(student_list, key=lambda student: student.grade, reverse=True)
    return [student for student in filtered_average if student.grade >= 1.0]
...
def sort_by_worst_grade(student_list: list) -> list:
    ...
    """
    filtered_average = sorted(student_list, key=lambda student: student.grade)
    return [student for student in filtered_average if student.grade >= 1.0]
