...
def find_fastest_growing(tweets: list) -> Tweet:
    ...
    """
    fastest_growing_tweet = None

    for tweet in tweets:
        rate = tweet.retweets / tweet.time
        if fastest_growing_tweet is None:
            fastest_growing_tweet = tweet
        if rate > fastest_growing_tweet.retweets / fastest_growing_tweet.time:
            fastest_growing_tweet = tweet
...
def sort_by_popularity(tweets: list) -> list:
    ...
    """
    sorted_list = sorted(tweets, key=lambda tweet: (tweet.retweets, -tweet.time), reverse=True)
    return sorted_list
...
def sort_hashtags_by_popularity(tweets: list) -> list:
    """
    Sort hashtags by popularity (sum of retweets across all tweets with that hashtag).

    If two hashtags have same popularity, sort by alphabet A-Z (uppercase first).
    """
    hashtag_popularity = {}
